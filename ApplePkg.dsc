...
DEFINE DASH_MACRO  = -DEFI_SPECIFICATION_VERSION=$(EFI_SPECIFICATION_VERSION) -DPI_SPECIFICATION_VERSION=$(PI_SPECIFICATION_VERSION) -DTIANO_RELEASE_VERSION=$(TIANO_RELEASE_VERSION)
  ...
  INTEL:RELEASE_*_*_CC_FLAGS = $(SLASH_MACRO) /D MDEPKG_NDEBUG /D EFI_NDEBUG
  GCC:DEBUG_*_*_CC_FLAGS     = $(DASH_MACRO) -DEFI_DEBUG
  GCC:RELEASE_*_*_CC_FLAGS   = $(DASH_MACRO) -DMDEPKG_NDEBUG -DEFI_NDEBUG
  MSFT:DEBUG_*_*_CC_FLAGS    = $(SLASH_MACRO) -DEFI_DEBUG
  MSFT:RELEASE_*_*_CC_FLAGS  = $(SLASH_MACRO) /D MDEPKG_NDEBUG /D EFI_NDEBUG
  XCODE:DEBUG_*_*_CC_FLAGS   = $(DASH_MACRO) -DEFI_DEBUG
  XCODE:RELEASE_*_*_CC_FLAGS = $(DASH_MACRO) -DMDEPKG_NDEBUG -DEFI_NDEBUG
